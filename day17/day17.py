...
type Direction = complex  # 1,-1,1j,-1j only
...
def dijkstra(source: Position, dest: Position, min_steps: int = 1, max_steps: int = 3) -> int:
    ...
    # init q
    q: list[PrioNode] = [PrioNode(0, (source, 1, 0)), PrioNode(0, (source, 1j, 0))]
    heapify(q)

    while q:
        prionode = heappop(q)
        cost, node = prionode.cost, prionode.node
...
p1 = dijkstra(source, dest, 1, 3)
p2 = dijkstra(source, dest, 4, 10)
print('Part 1:', p1)
print('Part 2:', p2)
