...
type Node = tuple[Position, Direction, int]  # int: number of straight steps
...
def valid_neighbors(node: Node) -> list[Node]:
    pos, d, s = node
    res = []
    if (left := pos + d*1j) in grid:
        res.append((left, d*1j, 1))
    if (right := pos - d*1j) in grid:
        res.append((right, -d*1j, 1))
    if (straight := pos + d) in grid and s < 3:
        res.append((straight, d, s+1))
    return res


# modified dijkstra
def dijkstra(source: Position, dest: Position) -> int:
    ...
    while q:
        ...
        for new_node in valid_neighbors(node):  # update valid neighbor
            new_pos = new_node[0]
